...
class MultiHmmCell(jit.ScriptModule):
    ...
    def __init__(self, n_states, n_outputs, n_chains):
        ...
        self.trans_logits_mu = nn.Parameter(th.randn(n_chains, n_states, n_states))
        self.obs_logits_mu = nn.Parameter(th.randn(n_chains, n_states, n_outputs))
        self.init_logits_mu = nn.Parameter(th.randn(n_chains, n_states))
        
        self.trans_logits_std = nn.Parameter(th.randn(n_chains, n_states, n_states))
        self.obs_logits_std = nn.Parameter(th.randn(n_chains, n_states, n_outputs))
        self.init_logits_std = nn.Parameter(th.randn(n_chains, n_states))
    ...
    def forward(self, obs: Tensor, chain: Tensor) -> Tensor:
        ...
        init_logits = th.normal(self.init_logits_mu, th.exp(self.init_logits_std))
        log_alpha = F.log_softmax(init_logits, dim=1) # n_chains x n_states

        obs_logits = th.normal(self.obs_logits_mu, th.exp(self.obs_logits_std))
        log_obs = F.log_softmax(obs_logits, dim=2) # n_chains x n_states x n_obs

        trans_logits = th.normal(self.trans_logits_mu, th.exp(self.trans_logits_std))
    ...
    for e in range(epochs):
        ...
        for offset in range(0, len(train_seqs), n_batch_seqs):
            ...
            for r in range(kwargs['n_train_samples']):
                ...
                active_kcs = hard_A.sum(0)
                active_kcs = active_kcs / (active_kcs + 1e-3) # K

                kc_stds = (active_kcs[:,None,None] * (th.exp(model.hmm.obs_logits_std))).mean() + \
                          (active_kcs[:,None,None] * (th.exp(model.hmm.init_logits_std))).mean() + \
                          (active_kcs[:,None,None] * (th.exp(model.hmm.trans_logits_std))).mean()
            ...
            #print(final_kc_stds.mean())

            train_loss = train_loss[mask_ix].mean() + kwargs['lambda'] * (final_kc_stds.mean())
