...
class BktModel(nn.Module):
    def __init__(self, n_kcs, n_latent_kcs, device='cuda:0'):
        ...
        self.membership_logits_mask = th.zeros((n_kcs, n_latent_kcs)).float().to(device)
        self.membership_logits_mask[:, 0] = 1 # only utilize the first latent KC initially

    def set_max_utilized_kcs(self, n_utilized_kcs):
        self.membership_logits_mask[:, :n_utilized_kcs] = 1
    
    def sample_A(self, tau, hard_samples):
        
        mask = self.membership_logits_mask 
        membership_logits_weight = self.kc_membership_logits.weight * mask + (1-mask) * -20
        
        self._A = nn.functional.gumbel_softmax(membership_logits_weight, hard=hard_samples, tau=tau, dim=1)
    ...
    def get_params(self):
        ...
        mask = self.membership_logits_mask 
        membership_logits_weight = self.kc_membership_logits.weight * mask + (1-mask) * -10
        kc_membership_probs = F.softmax(membership_logits_weight, dim=1) # n_problems * n_latent_kcs
...
def train(train_seqs, valid_seqs, n_kcs, device, learning_rate, epochs, n_batch_seqs, tau, **kwargs):
    ...
    for e in range(epochs):
        ...
        if waited == cfg['round_len']:
            utilized_kcs += 10
            model.set_max_utilized_kcs(utilized_kcs)
            waited = 0
            print("Inceasing KC utilization to %d" % utilized_kcs)
        else:
            waited += 1
