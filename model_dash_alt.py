...
class DashModel:
    ...
    def __init__(self, max_kc_id, cfg):
        self.p_batch_trials = cfg['p_batch_trials']
        ...
        self._logit_probs_kc_assignment = tf.Variable(tf.random.normal((self.n_kcs, self.n_groups), mean=0, stddev=0.1), name="logit_probs_kc_assignment")

    def get_trainables(self):
        trainables = []
        trainables.extend([(self._coeff.name,self._coeff), (self._logit_probs_kc_assignment.name, self._logit_probs_kc_assignment)])
    ...
    def train(self, train_df, valid_df):
        ...
        # train
        n_batch_trials = int(train_df.shape[0] * self.p_batch_trials)
        ...
        for e in range(self.epochs):
            ...
            valid_loss = self.evaluate(valid_df)
    ...
    def _run_model(self, F, curr_skill, testing=False):
        ...
        # [n_kcs, n_groups]
        exp = tf.math.exp(self._logit_probs_kc_assignment)
        S = exp / tf.reduce_sum(exp, axis=1, keepdims=True)
