...
class DKTModel(nn.Module):
    def __init__(self, n_kcs, n_hidden, n_items):
...
def train(train_seqs, 
    ...
    model = DKTModel(n_kcs, n_hidden, n_items)
    model = model.cuda()
    ...
    for e in range(epochs):
        ...
        for from_seq in range(0, len(train_seqs), n_batch_seqs):
            ...
            for i in range(0, n_trials, n_batch_trials):
                ...
                cell_input, curr_item, curr_correct, mask = transform(block, n_kcs, n_items)
                
                cell_input = cell_input.cuda()
                curr_item = curr_item.cuda()

                if i == 0:
                    logits, state = model(cell_input, curr_item)
                else:
                    hn, cn = state[0].detach(), state[1].detach()
                    logits, state = model(cell_input, curr_item, (hn, cn))
...
def predict(model, test_seqs, n_batch_seqs=50, n_batch_trials=100000):
    ...
    with th.no_grad():
        ...
        for seqs, new_seqs in sequences.iterate_batched(test_seqs, n_batch_seqs, n_batch_trials, pad_to_maximum=True):
            cell_input, curr_item, curr_correct, mask = transform(seqs, model.n_kcs, model.n_items)
...
def transform(subseqs, n_kcs, n_items):
    n_batch = len(subseqs)
...
def main(cfg_path, dataset_name, output_path):
    ...
    for s in range(splits.shape[0]):
        ...
        n_items = int(np.max(df['problem']) + 1)
        print("KCS: %d, Items: %d" % (n_obs_kcs, n_items))
