...
class MultiHmmCell(jit.ScriptModule):
    ...
    def __init__(self, n_states, n_outputs, n_chains, obs_abilities, trans_abilities):
        ...
        self.obs_abilities = obs_abilities # [n_obs, n_abilities]
        self.trans_abilities = trans_abilities # [n_states, n_abilities]
    ...
    def forward(self, obs: Tensor, chain: Tensor) -> Tensor:
        ...
        log_obs = F.log_softmax(self.obs_logits[:,:,:,None] + self.obs_abilities[None,None,:,:], dim=2) # n_chains x n_states x n_obs x n_abilities
        
        # n_chains x n_states x n_states x 1 + 1 x n_states x 1 x n_abilities
        log_t = F.log_softmax(self.trans_logits[:,:,:,None] + self.trans_abilities[None, :, None, :], dim=1) # n_chains x n_states x n_states x n_abilities
        
        # [n_batch, n_chains, n_states, n_abilities]
        log_alpha = th.tile(log_alpha, (n_batch, 1, 1))
        n_abilities = self.obs_abilities.shape[1]
        ...
        for i in range(0, obs.shape[1]):
            ...
            # B x 1 X S X A + B x 1 x S X A + B x S x S X A = B X S X S X A
...
class BktModel(nn.Module):
    def __init__(self, n_kcs, n_abilities, min_ability, max_ability):
        ...
        abilities = th.linspace(min_ability, max_ability, n_abilities)
        
        obs_abilities = th.repeat_interleave(abilities, n_abilities)
        obs_abilities = th.vstack((-obs_abilities, obs_abilities)).to('cuda:0')

        trans_abilities = th.tile(abilities, (n_abilities,))
        trans_abilities = th.vstack((-trans_abilities, trans_abilities)).to('cuda:0')
        
        self.hmm = MultiHmmCell(2, 2, n_kcs, obs_abilities, trans_abilities)
